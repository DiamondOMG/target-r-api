...
function GetScreenById() {
	...
	const getScreenById = async (e) => {
		e.preventDefault(); // ป้องกันการรีเฟรชหน้าเมื่อกดปุ่ม submit
		setIsLoading(true); // เริ่มโหลดข้อมูล
		setError(null); // เคลียร์ข้อผิดพลาดก่อนการโหลดใหม่

		try {
			const response = await fetch("/api/getScreenById", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
				body: JSON.stringify({ macid }), // ส่ง macid ไปยัง API
			});
			...
			setspecificScreen(data); // ดึงข้อมูลที่ได้จาก API มาเก็บใน state
		} catch (error) {
						...
						<form onSubmit={getScreenById}>
							<div className="relative z-10 flex gap-x-3 p-3 bg-white border rounded-lg shadow-lg shadow-gray-100 dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-gray-900/20">
								<div className="w-full">
									...
									>
										<span className="sr-only">Mac Adress</span>
									</label>
									<input
										type="text"
										...
										placeholder="Mac Address Id"
										value={macid}
										onChange={(e) => setmacid(e.target.value)} // อัปเดตค่า macid
										required // บังคับให้ใส่ค่า
									/>
								</div>
								<div>
									<button
